...
app.post("/api/khatas", authenticateJWT, (req, res) => {
  ...
  const query = "INSERT INTO khatas (`Name`,`userId`) VALUES (?, ?)";
...
app.get("/api/khatas", (req, res) => {
  const id = req.query.id;

  const query = "SELECT * FROM khatas WHERE userId = ?";
  db.query(query, [id], (err, results) => {
    if (err) {
      console.error("Error fetching khatas:", err);
      return res.status(500).json({ message: "Error fetching khatas" });
    }
    res.send(results);
  });
});
